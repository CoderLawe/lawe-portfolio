import Skill from "./Skill"
import TouchAppIcon from '@mui/icons-material/TouchApp';

const Skills = () => {
    return(
        <div>
          <h1 className="block gap-y-4  ml-20 text-headerYellow text-4xl ">Skills</h1>

          <div className="flex space-x-4 mt-5">

          <div className="group border-2 mx-12 cursor-pointer border-gray-500 lg:w-[200px] h-[150px] group-hover:h-[250px]">
                <div className="flex justify-center">
                    <div className="flex-col">
                        <TouchAppIcon className="h-20 w-20 text-gray-400" />
                        
                        <div className="hidden group-hover:block  transition-all duration-500 ease-in-out ">
                            <p className="text-gray-400 font-Cormorant mt-5">Responsive designs</p> 
                            <p className="text-gray-400 font-Cormorant mt-5">Lorem ipsum</p>

                        </div>
                    </div>

                </div>
              </div>
              <div className="border-2 mx-12  border-gray-500 lg:w-[200px] max-h-[250px]">
                <div className="flex justify-center">
                    <div className="flex-col">
                        <TouchAppIcon className=" h-20 w-20 text-gray-400" />
                        
                        <div>
                            <p className="text-gray-400 font-Cormorant mt-5">Lorem ipsum</p>
                        </div>
                    </div>

                </div>
              </div>
        
          </div>

          {/* <div className="flex mt-10 space-x-4">

              <div className="border-2 border-white lg:w-[250px] h-[250px]">
                <div className="flex justify-center">
                    <TouchAppIcon className=" h-32 w-32 text-gray-400" />

                </div>
              </div>

              <div className="border-2 border-white lg:w-[250px] h-[250px]">
                <div className="flex justify-center">
                    <TouchAppIcon className=" h-32 w-32 text-gray-400" />

                </div>
              </div>

        
          </div> */}

          {/* <div className="grid grid-cols-2 lg:ml-20 mx-5 lg:mx-0 gap-x-2 mt-6 gap-y-6 lg:mr-10  items-center align-middle">
                <img className="bg-cover rounded-full" src="https://cdn.discordapp.com/attachments/817048198022430761/950483294749884456/JavascriptLogo.png" alt="skill-logo"/>
                <img className="bg-cover rounded-full" src="https://cdn.discordapp.com/attachments/817048198022430761/950483294946996344/djangoLogo.png" alt="skill-logo"/>
                <img className="bg-cover rounded-full" src="https://cdn.discordapp.com/attachments/817048198022430761/950483295215448074/PythonLogo.png" alt="skill-logo"/>


                <img className="bg-cover rounded-full" src="https://cdn.discordapp.com/attachments/817048198022430761/950483295416754216/nextJS_logo.png" alt="skill-logo"/>
                 <img className="bg-cover h-36 w-40" src="https://cdn.discordapp.com/attachments/817048198022430761/950483294749884456/JavascriptLogo.png" alt="skill-tailwind-logo"/> 
                <img className="object-cover rounded-full" src="data:image/png;base64," alt="skill-logo"/>

          </div> */}
        </div>
    )
}

export default Skills